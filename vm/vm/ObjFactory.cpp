...
FObj * ObjFactory::allocObj(Env *env, FType *type, int addRef, int size) {
    ...
    FObj * obj = (FObj *)env->vm->gc.alloc(env, ftype, size);
    //obj->type = ftype;
...
FObj * sys_Int_box__(fr_Env self, fr_Int i, int addRef) {
    ...
    int size = sizeof(struct fr_ObjHeader) + sizeof(fr_Int);
    
    FObj * obj = e->allocObj(type, addRef, size);
    //FObj * obj = fr_allocObj(self, type, addRef, size);
    
    fr_Int *val = (fr_Int*)(((char*)obj) + sizeof(struct fr_ObjHeader));
    *val = i;
    return obj;
}
fr_Int sys_Int_unbox__(fr_Env self, FObj * i) {
    assert(i);
    fr_Int *val = (fr_Int*)(((char*)i) + sizeof(struct fr_ObjHeader));
    return *val;
}
...
FObj * sys_Float_box__(fr_Env self, fr_Float i, int addRef) {
    ...
    int size = sizeof(struct fr_ObjHeader) + sizeof(fr_Float);
    
    FObj * obj = e->allocObj(type, addRef, size);
    //FObj * obj = fr_allocObj(self, type, addRef, size);
    
    fr_Float *val = (fr_Float*)(((char*)obj) + sizeof(struct fr_ObjHeader));
    *val = i;
    return obj;
}
fr_Float sys_Float_unbox__(fr_Env self, FObj * i) {
    assert(i);
    fr_Float *val = (fr_Float*)(((char*)i) + sizeof(struct fr_ObjHeader));
    return *val;
}
...
FObj * sys_Bool_box__(fr_Env self, fr_Bool i, int addRef) {
    ...
    int size = sizeof(struct fr_ObjHeader) + sizeof(fr_Bool);
    
    FObj * obj = e->allocObj(type, addRef, size);
    //FObj * obj = fr_allocObj(self, type, addRef, size);
    
    fr_Bool *val = (fr_Bool*)(((char*)obj) + sizeof(struct fr_ObjHeader));
    *val = i;
    return obj;
}
fr_Bool sys_Bool_unbox__(fr_Env self, FObj * i) {
    assert(i);
    fr_Bool *val = (fr_Bool*)(((char*)i) + sizeof(struct fr_ObjHeader));
    return *val;
}
#endif
...
bool ObjFactory::unbox(Env *env, FObj * &obj, fr_Value &value) {
    fr_ValueType type = env->podManager->getValueTypeByType(env, (FType *)(obj->type));
    if (type == fr_vtInt) {
