...
#include "fni.h"
    ...
    struct fr_Class_ { struct FType *ftype; };
    typedef fr_Type fr_Class;
    ...
    fr_Class fr_getClass(fr_Env, fr_Obj obj);
    int fr_getFuncArity(fr_Env, fr_Class clz);
    const char *getTypeName(fr_Env, fr_Obj obj);
    struct FType *fr_getFType(fr_Env, fr_Obj obj);
...
#define FR_TYPE(type) (sys_Type)fr_sysType(__env, fr_findType(__env, NULL, #type))
#define FR_TYPE_IS(obj, type) fr_isInstanceOf(__env, obj, fr_findType(__env, NULL, #type))
...
#define FR_ALLOC(type) ((type##_ref)fr_malloc(__env, fr_findType(__env, NULL, #type)))
