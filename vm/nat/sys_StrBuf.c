...
void sys_StrBuf_capacity1(fr_Env __env, sys_StrBuf_ref __self, sys_Int it){
    ...
    wchar_t *p = realloc(__self->data, it+1);
    if (p) {
        __self->capacity = it;
        __self->data = p;
        if (__self->size > it) {
...
sys_Str sys_StrBuf_getRange1(fr_Env __env, sys_StrBuf_ref __self, sys_Range range){
    ...
    sys_Str str = fr_newStr(__env, __self->data+start, size);
    return str;
}
sys_This sys_StrBuf_set2(fr_Env __env, sys_StrBuf_ref __self, sys_Int index, sys_Int ch){
    if (index < 0) {
        index += __self->size;
    }
    if (index < 0 || index >= __self->size) {
    ...
    return (sys_This)__self;
}
sys_This sys_StrBuf_add1(fr_Env __env, sys_StrBuf_ref __self, sys_Obj_null x){
...
sys_This sys_StrBuf_addChar1(fr_Env __env, sys_StrBuf_ref __self, sys_Int ch){
...
//}
sys_This sys_StrBuf_join2(fr_Env __env, sys_StrBuf_ref __self, sys_Obj_null x, sys_Str sep){
    if (__self->size > 0) {
...
sys_This sys_StrBuf_insert2(fr_Env __env, sys_StrBuf_ref __self, sys_Int index, sys_Obj_null x){
    if (index < 0) {
        index += __self->size;
    }
    if (index < 0 || index > __self->size) {
...
sys_This sys_StrBuf_remove1(fr_Env __env, sys_StrBuf_ref __self, sys_Int index) {
...
sys_This sys_StrBuf_removeRange1(fr_Env __env, sys_StrBuf_ref __self, sys_Range range){
    sys_Int start = range->_start;
    sys_Int end = range->_end;
    if (start < 0) {
        start += __self->size;
...
sys_This sys_StrBuf_replaceRange2(fr_Env __env, sys_StrBuf_ref __self, sys_Range range, sys_Str str){
    sys_Int start = range->_start;
    sys_Int end = range->_end;
    if (start < 0) {
        start += __self->size;
...
sys_This sys_StrBuf_clear0(fr_Env __env, sys_StrBuf_ref __self){
    __self->size = 0;
    return (sys_This)__self;
}
sys_Str sys_StrBuf_toStr0(fr_Env __env, sys_StrBuf_ref __self){
    sys_Str str = fr_newStr(__env, __self->data, __self->size);
    return str;
}
