...
//////////////////////////////////////////////////////////////////////////
  ...
  ** is readonly safe.
  **
  Obj? getSafe(Int index, Obj? def := null) {
    if (index < 0) {
      index += size
    }
  ...
  **   list[1..-2]  => [1, 2]
  **
  @Operator FanList getRange(Range r) {
    start := r.start
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  @Operator FanList set(Int index, Obj? item) {
    modify
    if (index < 0) {
      index = size + index
    }
  ...
  ** ReadonlyErr if readonly.
  **
  FanList insert(Int index, Obj? item) {
    modify
  ...
  ** if readonly.
  **
  FanList insertAll(Int index, FanList list) {
    modify
    if (index < 0) index += size
    if (index > size) throw IndexErr("index is out of range")
  ...
  ** Throw ReadonlyErr if readonly.
  **
  FanList removeAll(FanList list) {
    modify
    for (i:=0; i<list.size; ++i) {
      obj := list[i]
      remove(obj)
  ...
  ** Throw ReadonlyErr if readonly.
  **
  FanList clear() { modify; size = 0; return this }
  ...
  **
  FanList trim() { modify; capacity = size; return this }
  ...
  **   Int[,].fill(0, 3)  =>  [0, 0, 0]
  **
  FanList fill(Obj? val, Int times) {
    modify
    grow(size + times)
    for (i:=0; i<times; ++i) {
      add(val)
...
//////////////////////////////////////////////////////////////////////////
  ...
  ** if readonly.
  **
  FanList push(Obj? item) { modify; return add(item) }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  Void eachr(|Obj? item, Int index| c){
    for (i:=size-1; i>=0; --i) {
      obj := this[i]
      c(obj, i)
    }
  ...
  **
  Void eachRange(Range r, |Obj? item, Int index| c) {
    start := r.start
    if (start < 0) start += size
    if (start >= size) throw IndexErr("range illegal")
    end := r.end
  ...
  **
  Obj? eachWhile(|Obj? item, Int index->Obj?| c) {
    for (i:=0; i<size; ++i) {
      obj := this[i]
      result := c(obj, i)
      if (result != null) {
  ...
  ** is readonly safe.
  **
  Obj? eachrWhile(|Obj? item, Int index->Obj?| c) {
    for (i:=size-1; i>=0; --i) {
      obj := this[i]
      result := c(obj, i)
      if (result != null) {
  ...
  **
  Obj? find(|Obj? item, Int index->Bool| c) {
    for (i:=0; i<size; ++i) {
      obj := this[i]
      result := c(obj, i)
      if (result) {
  ...
  **
  Int? findIndex(|Obj? item, Int index->Bool| c) {
    for (i:=0; i<size; ++i) {
      obj := this[i]
      result := c(obj, i)
      if (result) {
  ...
  **
  FanList findAll(|Obj? item, Int index->Bool| c) {
    nlist := FanList.make(of, 1)
    for (i:=0; i<size; ++i) {
      obj := this[i]
      result := c(obj, i)
      if (result) {
  ...
  **
  FanList exclude(|Obj? item, Int index->Bool| c) {
    nlist := FanList.make(of, 1)
    for (i:=0; i<size; ++i) {
      obj := this[i]
      result := c(obj, i)
  ...
  **
  Bool any(|Obj? item, Int index->Bool| c) {
    for (i:=0; i<size; ++i) {
      obj := this[i]
      result := c(obj, i)
      if (result) {
  ...
  **
  Bool all(|Obj? item, Int index->Bool| c) {
    for (i:=0; i<size; ++i) {
      obj := this[i]
      result := c(obj, i)
      if (!result) {
  ...
  **
  Obj?[] map(|Obj? item, Int index->Obj?| c) {
    nlist := List.make(Obj?#, size)
    for (i:=0; i<size; ++i) {
      obj := this[i]
      result := c(obj, i)
      nlist.add(result)
  ...
  **
  Obj? reduce(Obj? init, |Obj? reduction, Obj? item, Int index->Obj?| c) {
    Obj? reduction := init
    for (i:=0; i<size; ++i) {
      obj := this[i]
  ...
  **
  FanList intersection(FanList that) {
    //TODO
    throw FanUnsupportedErr()
  }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  FanList sort(|Obj? a, Obj? b->Int|? c := null) {
    //TODO
    throw FanUnsupportedErr()
  }
  ...
  **
  FanList sortr(|Obj? a, Obj? b->Int|? c := null) {
    //TODO
    throw FanUnsupportedErr()
  }
  ...
  **
  Int binarySearch(Obj? key, |Obj? a, Obj? b->Int|? c := null) {
    //TODO
    throw FanUnsupportedErr()
  }
  ...
  **
  Int binaryFind(|Obj? item, Int index->Int| c) {
    //TODO
    throw FanUnsupportedErr()
  }
  ...
  ** Throw ReadonlyErr if readonly.
  **
  FanList swap(Int indexA, Int indexB) {
    modify
  ...
  **
  FanList moveTo(Obj? item, Int toIndex) {
    modify
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  Str join(Str separator := "", |Obj? item, Int index->Str|? c := null) {
    buf := StrBuf()
    for (i:=0; i<size; ++i) {
      item := array[i]
