...
internal class FanMapEntry {
  Obj? key
  Obj? value

  FanMapEntry? next
  FanMapEntry? previous
}

internal class FanMapEntryList {
  private FanMapEntry head := FanMapEntry()
  private FanMapEntry tail := head
  ...
  FanMapEntry begin() { head }
  FanMapEntry end() { tail }

  FanMapEntry? findByKey(Obj? key) {
    FanMapEntry entry := begin
    while (entry != end) {
      if (entry.key == key) {
        return entry
      }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  new make(Type type) {
    this.type = type
    array = FanMapEntryList[,] { it.size = 100 }
  }
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  @Operator FanMap set(Obj? key, Obj? val) {
    modify
    hash := getHash(key)
    l := array[hash]
    if (l == null) {
  ...
  **   m  =>  [0:0, 1:1, 2:2]
  **
  FanMap setList(Obj?[] list, |Obj? item, Int index->Obj?|? c := null) {
    modify
  ...
  **   m  =>  [0:0, 1:1, 2:2]
  **
  FanMap addList(Obj?[] list, |Obj? item, Int index->Obj?|? c := null) {
    modify
    for (i:=0; i<list.size; ++i) {
      key := list[i]
      value := key
...
//////////////////////////////////////////////////////////////////////////
  ...
  **
  FanMap findAll(|Obj? val, Obj? key->Bool| c) {
    nmap := FanMap.make(type)
    for (i:=0; i<array.size; ++i) {
      l := array[i]
      if (l == null) continue
  ...
  **
  FanMap exclude(|Obj? val, Obj? key->Bool| c) {
    nmap := FanMap.make(type)
    for (i:=0; i<array.size; ++i) {
      l := array[i]
      if (l == null) continue
